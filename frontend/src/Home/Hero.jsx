        ...
        blogs.slice(0, 4).map((element) => {
          return (
            <Link
              to={`/blog/${element._id}`}
              key={element._id}
              className="bg-white rounded-lg hover:shadow-xl hover:shadow-neutral-800 overflow-hidden transform hover:scale-105 transition-transform duration-300"
            >
              <div className="group relative">
                ...
                <h1 className=" absolute bottom-4 left-4 text-white text-xl font-bold group-hover:text-blue-800 transition-colors duration-300">
                  {element.title}
                </h1>
              </div>
